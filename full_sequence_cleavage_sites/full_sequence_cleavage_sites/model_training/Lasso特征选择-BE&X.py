...
def load_features(use_x_features=False):
    ...
    be_range = (0,be_features.shape[1])
    ...
    base_features = be_features[selected_indices]

    if use_x_features and x_features is not None:
        x_range = (be_range[1], be_range[1] + x_features.shape[1])
...
def evaluate_model(model, X, y, cv=5):
    ...
    plt.legend()
    plt.savefig("../Picture/Lasso-5_AUC_all.svg", dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.savefig("../Picture/Lasso-5_AUPR_all.svg", dpi=300)
    plt.close()
