...
# Add a new import line for the scatter function from its dedicated library
...
#         # [NEW] 1. 门控网络 (Gating Network)
#         # 作用：计算每条边的重要性分数 (0到1之间)
#         # 相比 Attention (Softmax)，Sigmoid 允许保留多个重要特征，不会稀释信号
#         self.gate_mlp = nn.Sequential(
#             nn.Conv1d(edge_ind, hs, kernel_size=1, bias=bias),
#             nn.LeakyReLU(0.2),
#             nn.Conv1d(hs, len(aggr), kernel_size=1, bias=bias),
#             nn.Sigmoid() # [关键] 使用 Sigmoid 进行独立门控
#         )

#         # [NEW] 2. 层归一化 (LayerNorm)
#         # 作用：修复 Gating 导致的方差减小问题，防止梯度消失
#         self.layer_norm = nn.LayerNorm(hs * len(aggr))

#         # 原始 MLP1 (保持不变，用于处理 List 输入)
#         if edge_ind != 0:
#             self.mlp1 = nn.Sequential(
#                 nn.Conv1d(edge_attr_num * edge_ind * len(aggr), hs * len(aggr), kernel_size=1, bias=bias),
#                 nn.BatchNorm1d(hs * len(aggr)),
...
#     def compute_gated_aggregation(self, feat, edge_index_col, N):
#         """
#         核心函数：Gated Sum Aggregation
#         """
#         # [H100 Fix] 强制连续化，防止卡死
#         gate_input = feat.unsqueeze(0).permute(0, 2, 1).contiguous()
        
#         # 1. 计算门控值 [0, 1]
#         # output: [1, NumHeads, EdgeNum]
#         gate = self.gate_mlp(gate_input) 
        
#         # 还原维度: [EdgeNum, NumHeads]
#         gate = gate.squeeze(0).permute(1, 0).contiguous()

#         aggr_outputs = []
#         for i in range(len(self.aggr)):
#             # 取出第 i 个头的门控值
#             head_gate = gate[:, i].unsqueeze(1)
            
#             # [关键步骤] 加权：Feature * Gate
#             # 噪音会被 Gate (接近0) 过滤掉，信号会被 Gate (接近1) 保留
#             weighted_feat = feat * head_gate
            
#             # [关键步骤] 聚合：强制使用 Scatter Add (Sum)
#             # 因为实验证明 Sum > Max，我们这里只用 Sum
#             aggr_out = scatter_add(weighted_feat, edge_index_col, dim=0, dim_size=N)
#             aggr_outputs.append(aggr_out)

#         # 拼接
#         output = torch.cat(aggr_outputs, dim=1)
        
#         # [关键修复] 加上 LayerNorm 稳定数值
...
#         # === Case 1: edge_attr 是单个 Tensor ===
#         if isinstance(edge_attr, torch.Tensor):
#             row, col = edge_index
#             # 优先用 edge_attr，如果为 None 则用 x[row]
#             feat_in = edge_attr if edge_attr is not None else x[row]
            
#             # 使用 Gated Sum
#             aggr_out = self.compute_gated_aggregation(feat_in, col, N)
#             out.append(aggr_out)

#         # === Case 2: edge_attr 是 List (多图结构) ===
#         elif isinstance(edge_attr, list):
#             out_edge_list = []
#             for i in range(len(edge_index)):
#                 edge_index_i = edge_index[i]
...
#                 aggr_out_i = self.compute_gated_aggregation(feat_in, col, N)
#                 out_edge_list.append(aggr_out_i)

#             # 后处理：拼接 -> Permute -> MLP1 -> Permute
#             # [H100 Fix] 所有的 Permute 后都加了 contiguous
#             out_edge_stacked = torch.cat(out_edge_list, dim=1).unsqueeze(-1)
#             out_edge_stacked = out_edge_stacked.permute(2, 1, 0).contiguous()
            
#             out_edge_processed = self.mlp1(out_edge_stacked)
            
#             out_edge_processed = out_edge_processed.squeeze(0).permute(1, 0).contiguous()
#             out.append(out_edge_processed)

#         # === Final Combination ===
#         out = torch.cat(out, dim=1) 
        
#         # [H100 Fix] Permute -> contiguous
#         out_input = out.unsqueeze(0).permute(0, 2, 1).contiguous()
        
#         out = self.mlp2(out_input)
        
#         out = out.squeeze(0).permute(1, 0).contiguous()
#         return out
...
#         out = F.softmax(out, -1)
#         if hasattr(self, 'return_embeddings') and self.return_embeddings:
            
#             # 2. 如果开关打开，同时返回“最终得分”和“嵌入向量”
#             return out[:, 1], global_output
#         else:
#             # 3. 否则，保持原始行为，只返回得分
#             return out[:, 1]
#         #return out[:, 1]
