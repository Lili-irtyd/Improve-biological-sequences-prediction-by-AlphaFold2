...
def parse_args():
    ...
    parser.add_argument("--features", dest="features", default='PSSM,HMM,SS,AF, AF2',
...
def checkargs(args):
    ...
    for feature in features:
        if feature not in ['PSSM','HMM','SS','AF2','AF']:
...
class Config():
    def __init__(self,args):
        ...
        if 'PSSM' in features_list:
            self.feature_combine+='P'
        if 'HMM' in features_list:
            self.feature_combine+='H'
        if 'SS' in features_list:
            self.feature_combine+='S'
        if 'AF2' in features_list:
            self.feature_combine+='A2'
        if 'AF' in features_list:
...
def train(opt,device,model,learning_rate,train_data,valid_data,test_data,monitor=None):
    ...
    for epoch in range(epoch_begin,opt.epoch):
        ...
        for ii, data in enumerate(train_dataloader):
            ...
            if torch.isnan(data.x).any():
                print(f"警告：在批次 {ii} 中检测到输入数据 data.x 中有 nan！跳过此批次。")
                continue # 跳过这个损坏的批次
            if torch.isnan(data.y).any():
                print(f"警告：在批次 {ii} 中检测到输入数据 data.y 中有 nan！跳过此批次。")
                continue # 跳过这个损坏的批次
            model.train()
            data = data.to(device)
            # target = data.y.half()
            ...
            # ================= [新增] 可视化监控代码 =================
            if monitor is not None and ii % 100 == 0:
                # 计算当前的全局步数 (global step)
                current_step = epoch * nstep + ii
                # 记录统计量 (MAD, Variance, Histogram) 到 Tensorboard
                monitor.log_stats(current_step)
            # ========================================================
...
class FeatureMonitor:
    def __init__(self, writer):
        ...
        self.writer = writer
        self.step = 0
        self.layer_outputs = {} # 存储每一层的输出

    def _get_activation(self, name):
        def hook(model, input, output):
            # output 可能是 tuple，根据 NodeModel 的返回值，它可能是一个 Tensor
            if isinstance(output, tuple):
                data = output[0].detach()
            else:
                data = output.detach()
            
            self.layer_outputs[name] = data
        return hook

    def register_hooks(self, model):
        """
        针对你的 MetaBind_MultiEdges 结构注册 Hook
        重点监控 MetaGRU 中的 NodeModel 和 GlobalModel
        """
        # 假设使用的是 stacked_GN (MetaGRU)
        if hasattr(model.stacked_GN, 'node_model'):
            # 如果是 ModuleList (MetaGRUNoShareW 或 MetaComposed)
            if isinstance(model.stacked_GN.node_model, torch.nn.ModuleList):
                for i, layer in enumerate(model.stacked_GN.node_model):
                    handle = layer.register_forward_hook(self._get_activation(f'GRU_Step_{i}_Node'))
                    self.hooks.append(handle)
            # 如果是单个 Module (MetaGRU 共享权重)
            else:
                # 共享权重比较麻烦，Hook 会被调用多次，这里简化处理，只监控最后一次或覆盖
                handle = model.stacked_GN.node_model.register_forward_hook(self._get_activation('GRU_Shared_Node'))
                self.hooks.append(handle)

    def log_stats(self, step):
        """计算并记录统计量"""
        for name, feat in self.layer_outputs.items():
            # 1. 记录直方图：看分布是否健康
            self.writer.add_histogram(f'Distribution/{name}', feat, step)
            
            # 2. 计算 MAD (Mean Average Distance) - 衡量过平滑的核心指标
            # MAD 越小，说明节点特征越趋同（过平滑）
            # 为了计算快，我们只采样前 500 个节点
            if feat.size(0) > 500:
                feat_sample = feat[:500]
            else:
                feat_sample = feat
            
            # 计算两两距离矩阵
            dist = torch.cdist(feat_sample, feat_sample, p=2)
            mad_value = dist.mean().item()
            
            self.writer.add_scalar(f'Metric_MAD/{name}', mad_value, step)
            
            # 3. 记录特征的方差 (Variance)
            var_value = feat.var().item()
            self.writer.add_scalar(f'Metric_Var/{name}', var_value, step)

    def remove_hooks(self):
        for h in self.hooks:
            h.remove()
        self.hooks = [] # [新增] 清空列表，防止重注册时列表无限膨胀
...
def main(opt,device):
    ...
    tensorboard_log_dir = opt.model_path + '/tensorboard_logs'
    ...
    # ================= [新增] 初始化 Monitor 并注册 Hook =================
    print('Initializing Feature Monitor...')
    writer = SummaryWriter(log_dir=tensorboard_log_dir) # 实例化 Writer
    monitor = FeatureMonitor(writer)                    # 实例化 Monitor
    monitor.register_hooks(model)                       # 注册钩子到模型层
    print('Feature Monitor Registered. Logs will be saved to:', tensorboard_log_dir)
    # =================================================================
    ...
    if opt.train:
        print('=====train=====')
        train(opt, device, model, learning_rate, train_data, valid_data, test_data, monitor=monitor)
    print('=====test=====')
